...
def data_prepare(orig_path, haze_path):
    train_list = []
    val_list = []
    image_list_haze = glob.glob(haze_path + "*.jpg")
    ...
    for key in list(tmp_dict.keys()):
        ...
        if key in train_keys:
            for hazy_image in tmp_dict[key]:
                train_list.append([orig_path + key, haze_path + hazy_image])

        else:
            for hazy_image in tmp_dict[key]:
                val_list.append([orig_path + key, haze_path + hazy_image])
...
class dataset_loader(Data.Dataset):
    ...
    def __init__(self, orig_path, haze_path, mode='train'):
        self.train_list, self.val_list = data_prepare(orig_path, haze_path)
    ...
    def __getitem__(self, index):
        orig_data_path, haze_data_path = self.data_list[index]

        data_orig = Image.open(orig_data_path)
        data_hazy = Image.open(haze_data_path)
